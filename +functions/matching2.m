function matching2(h,e,MethodButtons,Axes,Save,IM,Operators)

Operators.Max_Iter = str2double(get(MethodButtons{2}.Edit{8}, 'String'));
Operators.TOL = str2double(get(MethodButtons{2}.Edit{10}, 'String')) / 100;
...
elseif Operators.TOL > 1
    warndlg(['The entered value for the maximum error (Max. Err.) is greater than expected. ', ...
             'Please review the input.'], 'Warning');
    [New_Motion, PSA_NM,PSV_NM,PSD_NM, Num_Iter, R] = functions.AlgGaussiano(Operators.TOL,Operators.Max_Iter,Operators);
...
set(MethodButtons{2}.Button{4},'Enable','off')

set(MethodButtons{2}.Button{3},'Enable','off')
